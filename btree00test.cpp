...
//         if(leaf == false){
//             fseek(fp, child[i]*BSIZE, SEEK_CUR);
//             fread(&temp, BSIZE, 1, fp);
...
void printKey(BTreeNode test){
    cout << "id: " << test.id << " key: ";
    for(int i=0; i<test.keycount; i++){
        cout << test.key[i] << " ";
    }
    cout << endl;
}
...
int main(){
    ...
    test1.load(fp, test.child[0]);
    test2.load(fp, test.child[1]);

    test3.load(fp, test1.child[0]);
    test4.load(fp, test1.child[1]);
    test5.load(fp, test1.child[2]);
    test6.load(fp, test1.child[3]);

    test7.load(fp, test2.child[0]);
    test8.load(fp, test2.child[1]);
    test9.load(fp, test2.child[2]);
